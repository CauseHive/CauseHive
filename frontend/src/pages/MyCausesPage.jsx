...
const MyCausesPage = () => {
  ...
  const fetchUserCauses = async () => {
    try {
      setLoading(true);
      const response = await apiService.getUserCauses();
      setCauses(response.results || []);
    } catch (err) {
      setError('Failed to load your causes. Please try again.');
      console.error('Error fetching user causes:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteCause = async (causeId) => {
    try {
      await apiService.deleteCause(causeId);
