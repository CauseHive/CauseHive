...
import { Heart, Target, Clock, Users, ChevronLeft, ChevronRight, Grid, List, SlidersHorizontal, ChevronDown } from 'lucide-react';
...
const CausesListPage = () => {
  ...
  const handleSearchSuggestionSelect = (suggestion) => {
    if (suggestion.type === 'cause' && suggestion.id) {
      // Navigate to specific cause
      navigate(`/causes/${suggestion.id}`);
    } else {
      // Use suggestion as search query
      const searchTerm = suggestion.title || suggestion.text || suggestion;
